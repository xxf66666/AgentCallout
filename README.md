...
v0.4.0 已发布：可选浏览器 DOM 定位（固定 playwright-core + 本机 Chrome，selector/文字/可访问性名称三模式，候选框绑定截图 SHA-256 与页面状态）通过 234 项测试、干净安装与双客户端真实"定位 → 批注 → 查看"验收。完整证据见[发布记录](docs/releases/0.4.0.md)与[网页元素定位文档](docs/dom.md)。
