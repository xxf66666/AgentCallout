...
| Claude Code 2.1.260  | Plugin 0.2.1 密集批注与修订 |
| Codex CLI 0.153.4    | MCP 0.2.1 密集批注与修订    |
| macOS、Linux         | 尚未完成项目级验证          |

v0.2.1 已发布：密集说明框避让、目标保护、折线引线、排版告警和预览像素指标通过了 179 项测试、干净安装及双客户端视觉 A/B。当前 Codex 使用兼容的 `auto` 显示标记；完整证据见[发布记录](docs/releases/0.2.1.md)。使用 GPT-6 时，已安装的 Codex CLI 0.152.1 可能提示升级，本版验收使用临时 0.153.4。
