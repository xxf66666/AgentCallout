...
The display hint is `auto`, matching current Codex image-bridge support. The byte and raster caps apply before host display; the hint does not promise a model token count. The renderer only protects supplied target geometry, so include adjacent control captions in the target when they also need protection, and inspect unmarked source text even when warnings are empty.
