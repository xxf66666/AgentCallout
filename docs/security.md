...
`agent-callout edit` 是可选的本地人工编辑入口：它只绑定回环地址 `127.0.0.1`，为每个会话生成高熵 URL token，并以无缓存、同源资源策略和限制型 CSP 提供页面。它不会加载第三方 URL、分析脚本或遥测；原图只在本机浏览器与本机 editor 进程之间传递。
...
- 编辑器 project 必须使用 `.json` 扩展名并为常规文件，不能是符号链接、目录或原图的 hard-link 别名；编辑器只会原子替换这个 project 文件。编辑器导出仍复用常规 PNG/sidecar 的路径校验与原图保护。
