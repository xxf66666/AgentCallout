...
图片扩展名不是信任依据。实现必须同时限制允许的 PNG/JPEG/WebP 扩展名、解码器识别格式和实际 magic bytes，并拒绝多帧或异常输入超出产品限制的情况。

### 2.3 输出与覆盖

- 原图永不作为输出目标。
- 默认拒绝已存在的输出 PNG 或 sidecar，不能静默递增文件名后假装使用了请求路径。
- 如果以后提供显式 overwrite，必须同时满足：调用方明确开启、目标仍在输出根目录、目标不是输入文件，并在写入前再次校验。
- 先在同一输出目录创建排他临时文件，完整写入、重新解码验证并同步后再原子提交；失败时删除临时文件，不留下半成品。
- PNG 与 sidecar 应作为同一逻辑事务处理。任一最终目标冲突或验证失败时，都不能把另一文件报告为成功。
...
| 限制 | 必须执行的位置 | 当前状态 |
| --- | --- | --- |
| 输入文件字节数 | 打开/读取前检查文件大小，流式读取仍设置上限 | **数值待实现与基准测试，发布前必须固定** |
| 单边最大宽高 | 读取可信 metadata 后、分配完整画布前 | **数值待实现与测试** |
| 解码后总像素 | Sharp `limitInputPixels` 与应用层同时约束 | **数值待实现与测试** |
| crop 与输出总像素 | 坐标转换后、渲染前再次计算 | **数值待实现与测试** |
| annotation 数量与单条/总文字长度 | Schema 验证阶段 | **数值待实现与测试** |
| MCP ImageContent/预览字节数 | base64 编码前；超限时生成受控预览并保留本地完整 PNG | **数值待实现与宿主测试** |
