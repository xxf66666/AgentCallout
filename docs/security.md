...
> v0.2.1 已完成 Windows 179 项测试、干净安装与 Claude/Codex 实测，详见[发布记录](releases/0.2.1.md)；该证据不外推到其他平台或全部截图。
...
| MCP ImageContent/预览            | base64 前逐级缩小；完整 PNG 仍保留在本地路径     | **64 KiB；最长边 512 px；显示 hint 为 auto** |
...
OCR 运行时必须由用户显式安装，普通识别不得自动下载或修复依赖。安装内容使用固定 lockfile、语言模型大小和 SHA-256 校验；识别在隔离子进程中执行，清理 `NODE_OPTIONS` 等注入入口，并禁止网络、DNS 和继续创建子进程。一次只运行一个识别，最多排队四个请求；输入图片、处理像素、OCR 行/词/符号、返回文字和超时均有硬上限。

MCP 请求不能指定运行时执行目录。OCR 不返回完整 transcript，也不把 confidence 当作正确概率；候选必须绑定输入 hash。多个候选、低置信度、截图变化或运行时变化都应显式失败或要求确认。OCR 文字 bbox 不是完整控件边界，也不承担敏感信息自动发现承诺。
