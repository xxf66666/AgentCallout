...
| Claude Code 安装/升级/卸载 | Git marketplace + plugin                                              | **PARTIALLY VERIFIED** | 两命令安装成功；0.1.0 → 0.1.1 update 成功；Plugin cache Sharp 与 bundle hash 验证                        | **PENDING：** 最终版本卸载并重装                          |
...
| Codex 安装/升级/卸载       | GitHub 全局 CLI + `codex mcp add`                                     | **PARTIALLY VERIFIED** | `--install-links=true` GitHub 安装、doctor/self-test、MCP 注册和真实 Agent 调用通过                      | **PENDING：** 最终版本 MCP remove/npm uninstall 后重装    |
| Codex 可选 Skill Plugin    | Git marketplace + Skills-only Plugin                                  | **PARTIALLY VERIFIED** | marketplace add、plugin add/upgrade/remove 与 Skill 发现均执行；0.1.2 已移除不可用 runtime 声明          | **PENDING：** 最终 Skills-only 版本 add                   |
...
| CLAUDE-002  | 2026-08-30 | README 两命令 GitHub 安装、update、headless stream-json        | Plugin 0.1.1、Skill 和 MCP connected；doctor 成功；两次 annotate 输出 `864b…1d30`/`f6cc…3db8`，两个预览可见并有视觉评价            | 最终 0.1.2 卸载/重装待执行                                            |
| CODEX-002   | 2026-08-30 | GitHub 全局安装 + `codex mcp add` + `codex exec --json`        | doctor/inspect/validate 成功；两次 annotate 输出 `e084…9430`/`59d9…cc83`，ImageContent 41,281 bytes，两个预览可见                  | 使用 Node 24/npm 11；最终卸载/重装待执行                              |
