...
- **兼容输出**：非图片工具返回 structuredContent + TextContent；图片工具根据客户端兼容性返回受限 ImageContent，并始终提供 JSON TextContent、sidecar 和本地路径。ResourceLink 延后到安全实现 `resources/read` 后。
