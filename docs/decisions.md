...
| D-006 | revision 默认只返回 touched/连带重排的单张聚焦预览；分散/全局/过大回退 compact-overview，敏感覆盖削弱则零图片。另提供 path/text/hash-free 的 sidecar 校验摘要                                                  | 减少重复 crop 和默认数据披露；代价是父 spec 需本地重渲染几何，聚焦视图不能代替全局复核         | 十类实际像素、隐私抑制、4 KiB 摘要、入口一致性和 inspect no-write 自动化已通过；真实客户端 A/B 待发布验证 | [ADR-0007](adr/0007-focused-review-and-safe-sidecar-summary.md)                                              |
...
7. [ ] 在 0.2.0 发布构建上验证 changed-region 能减少额外 crop，并验证 `inspect_annotation_sidecar` 不泄漏默认排除字段。
