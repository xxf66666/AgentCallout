...
| D-005 | 可编辑纯色遮挡以 **localhost Konva 编辑层 + Sharp 原图重渲染** 实现；编辑项目与导出 manifest 分离                                                                                                              | 增加浏览器 bundle，换取可直接拖拽/缩放且不把原图作为隐藏图层嵌入交付文件                    | Windows 本地 UI、导出和像素测试已验证；跨平台仍待回归                                 | [ADR-0006](adr/0006-editable-redaction-editor.md)                                                            |
